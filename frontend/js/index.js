...
const p = document.querySelector(".p"); // получаем текст по названию класса из main.css
const error = document.querySelector(".error"); // получаем текст по названию класса из main.css
p.className += " display_none"; // добавляем к элементу класс со стилями из main.css
...
elements = [ // список с названиями процедур
    "Нижняя ветка тройничного нерва",
    "Верхнее веко",
...
btn1.addEventListener("click", () => { // функция, которая срабатывает при нажатии на кнопку "Выбрать процедуру"
    if (count1 % 2 == 0) {
        ...
        for (let i = 0; i < elements.length; i++) { // циклом идём по списку процедур
            ...
            radio.value = i; // кнопке присваиваем значение
...
btn2.addEventListener("click", () => { // функция, которая срабатывает при нажатии на кнопку "Приступить"
    if (!flag) { // знак ! эквивалентен not в python
        error.className = "error"; // элементу устанавливаем класс
    } else {
        error.className += " display_none"; // элементу добавляем класс
